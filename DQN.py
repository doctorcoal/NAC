...
def build_net(layer_shape, activation, output_activation):
    ...
    for j in range(len(layer_shape)-1):
        act = activation if j < len(layer_shape)-2 else output_activation # only layer_shape - 2 (last layer) use output_activation
...
class Q_Net(nn.Module):
    def __init__(self, state_dim, action_dim, hid_shape):
        ...
        # with torch.no_grad():   # higher prob. for default action
        #     self.Q[-2].weight[0, :] += 1.0
        #     self.Q[-2].bias[0] += 1.0
...
class DQN_agent(object):
    ...
    def select_action(self, state, deterministic, mask=None):#only used when interact with the env
        def convert_mask(mask, dtype, dvc):
            if not mask: 
                return 0
            min_dtype = torch.finfo(dtype).min
            # return torch.tensor([False]+list(mask.values()), device=dvc).int() * min_dtype  # if not comp_full['F']
            return torch.tensor(list(mask.values()), device=dvc).int() * min_dtype
        with torch.no_grad():
            state = torch.FloatTensor(state.reshape(1, -1)).to(self.dvc)
            if deterministic:
                output = self.q_net(state)
                a = (output + convert_mask(mask, output.dtype, output.device)).argmax().item()
            else:
    ...
    def save(self,algo,EnvName,steps,foldername='model'):
        path=os.path.join('.',foldername)
        if not os.path.exists(path):
            os.makedirs(path)
        utils.save_on_master(self.q_net.state_dict(), os.path.join(path,'{}_{}_{}.pth'.format(algo,EnvName,steps)))
